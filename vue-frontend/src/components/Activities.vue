<template>
  <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
    <div v-if="loading" class="col-span-full text-center">Loading...</div>
    <div v-if="error" class="col-span-full text-center text-red-500">{{ error }}</div>
    <div v-for="activity in activities" :key="activity.id"
      ...
      <p class="text-gray-700 mb-1">Supplier Name: {{ activity.supplier.name }} </p>
      <p class="text-gray-700 mb-1">Supplier Location: {{ activity.supplier.location }} </p>
    </div>
  </div>
</template>
